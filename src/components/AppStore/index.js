...
import AppDetails from '../AppItem'
...
class App extends Component {
  state = {searchInput: '', tabItem: appsList}

  displayTab = tabId => {
    const SelectedTabItem = appsList.filter(
      eachTabItem => eachTabItem.category === tabId,
    )
    this.setState({tabItem: SelectedTabItem})
  }
  ...
  render() {
    const {searchInput, tabItem} = this.state

    const result = tabItem.filter(eachTab =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="app-list">
          {result.map(appItems => (
            <AppDetails key={appItems.appId} appItems={appItems} />
          ))}
        </ul>
      </div>
    )
